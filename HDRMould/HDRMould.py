...
class HDRMould:
  ...
  def __init__(self, parent):
    parent.title = "HDR Mould Generator"
    parent.categories = ["Brachytherapy"]
    parent.dependencies = []
    parent.contributors = ["Mark Schumacher (PerkLab)", "Jacob Andreou (PerkLab)", "Ian Cumming (PerkLab)", "Andras Lasso (PerkLab)"]
    parent.helpText = """
...
class HDRMouldWidget:
  ...
  def setup(self):
    ...
    self.reloadAndTestButton.setToolTip("Reload this module and then run the self tests.")
    ...
    self.createTestObjectButton.toolTip = ("Creates a mask of a specified thickness from the face of interest. Catheter paths are carved out"
      " along the line connecting the pairs of fiducials. This overwrites the Output Model.")
    ...
    self.surfaceModelComboBox.setToolTip("Choose the skin surface model for the surface mould to be made for")
    parametersFormLayout.addRow("Skin Surface Model ", self.surfaceModelComboBox)
    ...
    self.ouputMouldComboBox.setToolTip( "This model will be overwritten with the output surface mould" )
    parametersFormLayout.addRow("Output Mould Model ", self.ouputMouldComboBox)

    # ROI node comboBox - drop down menu
    self.roiComboBox = slicer.qMRMLNodeComboBox()
    self.roiComboBox.nodeTypes = ["vtkMRMLMarkupsROINode"]
    ...
    self.roiComboBox.setToolTip("Choose the area of the skin surface that the mould is to be made for. This determines the final size of the mould. ")
    parametersFormLayout.addRow("Mould Boundary", self.roiComboBox)

    # Ruler node comboBox - drop down menu
    self.rulerComboBox = slicer.qMRMLNodeComboBox()
    self.rulerComboBox.nodeTypes = ["vtkMRMLMarkupsLineNode"]
    ...
    self.rulerComboBox.setToolTip("This ruler and the pairs of points, will be used to define a plane used in path generation")
    parametersFormLayout.addRow("Normal direction ", self.rulerComboBox)
    ...
    self.startPointComboBox.setToolTip("Choose the markup list that contains the starting points of the catheter paths")
    parametersFormLayout.addRow("Catheter Start Points ", self.startPointComboBox)
    ...
    self.endPointComboBox.setToolTip("Choose the markup list node that, contains the end points of the catheter paths")
    ...
    self.minCathDistanceSlider.setToolTip("The closest absolute distance a catheter is aloud to get to the skin")
    self.minCathDistanceSlider.minimum = 1.5
    self.minCathDistanceSlider.maximum = 10
    self.minCathDistanceSlider.value = 2
    parametersFormLayout.addRow("Distance from Skin", self.minCathDistanceSlider)

    # Minimum catheter distance from target surface - slider and spinbox
    self.slitThickness = ctk.ctkSliderWidget()
    self.slitThickness.setToolTip("The thickness of the slit instantiated into the mould.")
    ...
    self.cathRadiusSlider.toolTip = "Defines the radius of the catheter tubes, in mm. Default: 2mm"
    ...
    self.minCurvatureSlider.toolTip = "Defines the minimum radius of curvature of any point along the catheter paths, in mm."
    self.minCurvatureSlider.minimum = 0
    self.minCurvatureSlider.maximum = 60
    self.minCurvatureSlider.value = 14
    parametersFormLayout.addRow("Catheter Curvature ", self.minCurvatureSlider)

    # Output mould model resolution value - slider and spinbox
    self.algorithm = ctk.ctkSliderWidget()
    self.algorithm.toolTip = "How Many Cycles to smooth"
    self.algorithm.minimum = 0
    self.algorithm.maximum = 1000
    self.algorithm.value = 500
    parametersFormLayout.addRow("algorithmRunning ", self.algorithm)


    # Make Brachytherapy Surface Mould - button
    self.createMouldButton = qt.QPushButton()
    self.createMouldButton.setText("Generate Mould")
    self.createMouldButton.toolTip = ("Creates a mask of a specified thickness from the face of interest. Catheter paths are carved out"
      " along the line connecting the pairs of fiducials. This overwrites the Output Model.")
  ...
  def logProgress(self, text):
    slicer.util.showStatusMessage(text, duration=5000)
  ...
  def CreateMould(self):
    ...
    self.logProgress("Clip input model...")
    ...
    #clippedFace is the algorithm Object
    if DEBUG_CLIPPING:
      node=utility.DisplayPolyData("Clipped-Data",mouldLogic.clippedSurface)
      node.EdgeVisibilityOn()
      utility.PolyDataWriter(mouldLogic.minimumDistanceMask, PATHDEBUG+"ClippedData.vtk")

    print("MouldLogic::MinimumDistanceMask()")
    self.logProgress("Compute offset surface...")
    ...
    if DEBUG_MINMASK:
      ...
      utility.PolyDataWriter(mouldLogic.minimumDistanceMask,PATHDEBUG+"MinMask.vtk")
    ...
    self.logProgress("Create catheter planes...")
    ...
    self.logProgress("Create catheter back lines...")
    ...
    for i in range (len(mouldLogic.listOfBackLines)):
      print(f"CatheterPath::Initializing for Plane {i}")
      self.logProgress(f"Create catheter path {i+1}...")
      ...
      catheterPath=CatheterPath(mouldLogic.minimumDistanceMask,
          ...
          CATHETER_DEBUG_SMOOTHEDPATH, CATHETER_DEBUG_TUBE, CATHETER_DEBUG_BADPOINTS, PATHDEBUG, self.algorithm.value)
    ...
    for i in range (len(mouldLogic.tubes)):
      self.logProgress(f"Rasterize catheter path {i+1}...")
    ...
    self.logProgress("Create mould model...")
    # Save mould into label volume
    mouldVolume = utility.DisplayImageData(completedMould,referenceVolume, "completedMould")
    # Convert to segmentation node
    mouldSegmentation = slicer.mrmlScene.AddNewNodeByClass("vtkMRMLSegmentationNode")
    slicer.modules.segmentations.logic().ImportLabelmapToSegmentationNode(mouldVolume , mouldSegmentation)
    slicer.mrmlScene.RemoveNode(mouldVolume)
    # Convert to model node
    mouldSegmentation.CreateClosedSurfaceRepresentation()
    segmentID = mouldSegmentation.GetSegmentation().GetNthSegmentID(0)
    mouldPolyData = vtk.vtkPolyData()
    mouldSegmentation.GetClosedSurfaceRepresentation(segmentID, mouldPolyData)
    self.ouputMouldComboBox.currentNode().SetAndObservePolyData(mouldPolyData)
    slicer.mrmlScene.RemoveNode(mouldSegmentation)
    ...
    self.logProgress("Mould generation completed.")
